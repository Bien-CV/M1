/*
  ...
  g++ -Wall -std=gnu++11 -o floyd-sequential floyd-sequential.cpp -fopenmp
or
  g++ -Wall -std=gnu++0x -o floyd-sequential floyd-sequential.cpp -fopenmp
		...
		const IMatrix& next, uint32_t i, uint32_t j)
{
  ...
  if (intermediate == -1) { // Edge i--j is shortest path
    ...
    return p;
  	...
  	path_t l,r;
/*  	#pragma omp parallel if (nbthread<2)
  	{
  		nbthread +=2;
	  	#pragma omp sections
	  	{
	  		#pragma omp section*/
			l= recover_path(dist,next,i,intermediate);
		//	#pragma omp section
			r = recover_path(dist,next,intermediate,j);
		/*}
		nbthread -=2;
	}*/
...
DMatrix generate_matrix(uint32_t sz)
{
  ...
  for (uint32_t i = 0; i < sz; ++i) {
    for (uint32_t j = 0; j < sz; ++j) {
      ...
      drand48_r(&buf, &W(i,j));
      if ((drand48_r(&buf2, &W(i,j)) < 2)) { // Introducing some sparsity
	W(i,j) = inf;
      }
    }
  }
