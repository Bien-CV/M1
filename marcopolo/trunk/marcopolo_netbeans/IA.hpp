 ...
 * @class IA
 *
 * @brief Classe abstraite controlant le deplacement du robot
 *
...
public:
    ...
    /**
     * \brief Controle les actions du robot.
     * */
    virtual void miseEnMarche() = 0;
    
    /**
     * \brief Calcule le plus court chemin avec l'algorithme A*
     * @param destination, la cellule de la carte désirée
     * @return un vector qui contient les direction à prendre, cellule par cellule
     */
    vector<int> itineraireAvecAStar(int destination);
    
    /**
     * \brief Calcule la position de la cellule non visitée la plus proche
     * @return l'identifiant de cette cellule
     */
    int celluleLaPlusProcheNonVisitee();
    
    /**
     * \brief Calcule la position de la cellule non visitée la plus éloignée
     * @return l'identifiant de cette cellule
     */
    int celluleLaPlusLoinNonVisitee();
